...
import (
	...
	menfessDto "anoa.com/telkomalumiforum/internal/modules/menfess/dto"
	menfessRepo "anoa.com/telkomalumiforum/internal/modules/menfess/repository"
	"anoa.com/telkomalumiforum/internal/modules/reaction/service"
...
type MenfessService interface {
	...
	GetMenfesses(ctx context.Context, userID *uuid.UUID, offset, limit int) ([]menfessDto.MenfessResponse, int64, error)
}

type menfessService struct {
	repo            menfessRepo.MenfessRepository
	reactionService reaction.ReactionService
	redisClient     *redis.Client
}

func NewMenfessService(repo menfessRepo.MenfessRepository, reactionService reaction.ReactionService, redisClient *redis.Client) MenfessService {
...
func (s *menfessService) GetMenfesses(ctx context.Context, userID *uuid.UUID, offset, limit int) ([]menfessDto.MenfessResponse, int64, error) {
	menfesses, total, err := s.repo.FindAll(ctx, offset, limit)
	if err != nil {
		return nil, 0, err
	}

	var responses []menfessDto.MenfessResponse
	for _, m := range menfesses {
		reactions, _ := s.reactionService.GetReactions(ctx, userID, m.ID, "menfess")

		responses = append(responses, menfessDto.MenfessResponse{
			ID:        m.ID.String(),
			Content:   m.Content,
			Reactions: *reactions,
