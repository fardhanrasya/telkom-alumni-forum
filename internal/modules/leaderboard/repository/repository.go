...
type LeaderboardRepository interface {
	CreatePointLog(log *entity.PointLog) error
	UpdateUserStats(userID uuid.UUID, points int) error
	GetDailyThreadCount(userID uuid.UUID, date time.Time) (int64, error)
	GetTopUsers(limit int, timeframe string) ([]entity.UserStats, error)
	GetUserStatsByUserID(userID uuid.UUID) (*entity.UserStats, error)
...
func (r *leaderboardRepository) CreatePointLog(log *entity.PointLog) error {
...
func (r *leaderboardRepository) GetDailyThreadCount(userID uuid.UUID, date time.Time) (int64, error) {
	...
	err := r.db.Model(&entity.PointLog{}).
		Where("user_id = ? AND action_type = ? AND created_at >= ? AND created_at < ?", userID, "create_thread", startOfDay, endOfDay).
		Count(&count).Error
	return count, err
}

func (r *leaderboardRepository) GetTopUsers(limit int, timeframe string) ([]entity.UserStats, error) {
	...
	var realStats []entity.UserStats
...
func (r *leaderboardRepository) GetUserStatsByUserID(userID uuid.UUID) (*entity.UserStats, error) {
	var stats entity.UserStats
