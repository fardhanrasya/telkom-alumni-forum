...
type MeiliSearchService interface {
	IndexThread(thread *entity.Thread) error
	IndexPost(post *entity.Post) error
...
func (s *meiliSearchService) IndexThread(thread *entity.Thread) error {
	allowedRoles := []string{}
	...
	doc := meiliThreadDoc{
		...
		CategoryID:   thread.CategoryID.String(), // Dereference if needed, but assuming model has *UUID handled or assuming it's UUID value. Double check entity.
	...
	// Double check CategoryID type in entity.
...
func (s *meiliSearchService) IndexPost(post *entity.Post) error {
