...
type ReactionService interface {
	ToggleReaction(ctx context.Context, userID uuid.UUID, req dto.ReactionToggleRequest) error
...
func NewReactionService(repo repository.ReactionRepository, redisClient *redis.Client, leaderboardService LeaderboardService, notificationService NotificationService, threadRepo repository.ThreadRepository, postRepo repository.PostRepository) ReactionService {
...
func (s *reactionService) ToggleReaction(ctx context.Context, userID uuid.UUID, req dto.ReactionToggleRequest) error {
	...
	if newEmoji != "" && req.ReferenceType != "menfess" {
		go func() {
			...
			if s.leaderboardService != nil {
				s.leaderboardService.AddGamificationPointsAsync(authorID, ActionLikeReceived, req.ReferenceID.String(), refTable, &userID)
			}
...
func (s *reactionService) GetReactions(ctx context.Context, userID *uuid.UUID, refID uuid.UUID, refType string) (*dto.ReactionsResponse, error) {
	...
	if userID != nil {
		userReacted, err = s.repo.GetUserReactions(ctx, *userID, refID, refType)
		if err != nil {
			return nil, err
		}
	} else {
